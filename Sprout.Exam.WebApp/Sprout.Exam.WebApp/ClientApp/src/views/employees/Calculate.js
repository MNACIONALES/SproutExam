...
export class EmployeeCalculate extends Component {
    ...
    constructor(props) {
        super(props);
        this.state = {
            id: 0, fullName: '', birthdate: '', tin: '', employeeTypeName: '', absentDays: 0, workedDays: 0, netIncome: 0, salaryComputation: {}, loading: true, loadingCalculate: false
        };
    }
        ...
        let contents = this.state.loading
            ...
            : <div>
                <form>
                    ...
                    <div className="form-row">
                        <div className='form-group col-md-12'>
                            <label>Employee Type: <b>{this.state.employeeTypeName}</b></label>
                        </div>
                    </div>

                    <div className="form-row">
                        <div className='form-group col-md-12'><label>{this.state.salaryComputation.rate} Rate: {this.state.salary} </label></div>
                        {this.state.salaryComputation.rate === "Monthly" ?
                            <div className='form-group col-md-12'><label>Tax: 12% </label></div> : <div></div>}
                    </div>
    ...
    async getEmployee(id) {
        ...
        if (response.status === 200) {
            ...
            this.setState({
                ...
                employeeTypeName: await this.getEmployeeType(data.employeeTypeId),
                salaryComputation: await this.getSalaryComputation(data.employeeTypeId),
                loading: false,
    ...
    async getSalaryComputation(employeeTypeId) {
        const token = await authService.getAccessToken();
        const response = await fetch('api/SalaryComputation/GetByEmployeeType/' + employeeTypeId, {
            headers: !token ? {} : { 'Authorization': `Bearer ${token}` }
        });
        const data = await response.json();
        return data;
