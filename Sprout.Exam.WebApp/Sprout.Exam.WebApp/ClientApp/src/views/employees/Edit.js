...
export class EmployeeEdit extends Component {
    ...
    constructor(props) {
        super(props);
        this.state = { id: 0, fullName: '', birthdate: '', tin: '', salary: '', typeId: 1, employeeTypes: [], loading: true, loadingSave: false };
    }
   ...
   async componentDidMount() {
                ...
                <form>
                    ...
                    <div className="form-row">
                        ...
                        <div className='form-group col-md-6'>
                            ...
                            {/*</select>*/}
                            <select onChange={this.handleChange.bind(this)} value={this.state.employeeTypeId} name="employeeTypeId" className='form-control'>
                                {this.state.employeeTypes &&
                                    this.state.employeeTypes.map((item) => {
                                        return (
                                            <option
                                                key={item.id}
                                                value={item.id}>
    ...
    async getEmployee(id) {
        ...
        this.setState({ id: data.id, fullName: data.fullName, birthdate: data.birthdate, tin: data.tin, employeeTypeId: data.employeeTypeId, salary: data.salary, loading: false, loadingSave: false });
